...
- `docker-ecr-setup.sh`
    ...
    The script uses the `aws ecr create-repository` command to create/initialize a Docker container
    repository into which Docker images can be pushed or from which the images can be pulled.

    usage: `docker-ecr-setup.sh <repositoryName>`

    Example invocation:

    ```bash
    docker-ecr-setup.sh ub-vuln
    ```
- `push-to-ecr.sh`

    This script uses the `docker tag` and `docker push` commands to mark and then push a docker image in the local
    registry to the specified AWS ECR repository.  The docker environment must be logged into AWS ECR for this command
    to work (see `docker-ecr-login.sh`).
    ...
    usage: `push-to-ecr.sh <repositoryName> <repositoryTag>`

    Example invocation:

    ```bash
    push-to-ecr.sh ub-vuln badImage
    ```
